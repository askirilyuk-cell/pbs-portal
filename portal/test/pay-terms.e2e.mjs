...
async function main() {
  ...
  try {
    ...
    await pay(POST());
    inv()['Ожидаемая дата оплаты'] = '2026-08-20';         // человек перенёс срок с поставщиком
    ...
    check('массовый опрос: расхождение видно в ответе', (ra.j.warnings || 0) > 0 || (Array.isArray(ra.j.warningList) && ra.j.warningList.length > 0), JSON.stringify(ra.j));
